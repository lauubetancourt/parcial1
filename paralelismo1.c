...
#include <stdlib.h>
#include <string.h>
#include <omp.h>  // Incluir librería OpenMP
...
int main(int argc, char* argv[]) {
    ...
    // Obtener el número de núcleos disponibles en la máquina
    ...
    // Aplicar filtro (paralelizable con OpenMP)
    #pragma omp parallel for
    for (int y = 1; y < height - 1; y++) {
        aplicarFiltro(imagen, imagenProcesada, width, height);
    }

    // Calcular suma de píxeles (paralelizable con OpenMP)
    int sumaPixeles = 0;
    #pragma omp parallel for reduction(+:sumaPixeles)
    for (int i = 0; i < width * height; i++) {
        sumaPixeles += imagenProcesada[i];
    }
